...
public class DataList<T> implements Serializable{
	...
	@JSONField(ordinal=3)
	private String apiId="";// API id 请求的唯一标识
	
	@JSONField(ordinal=4)
	private Integer totalCount=0;//记录总数
	
	@JSONField(ordinal=5)
	private List<T> rows=Collections.EMPTY_LIST;//本次返回的数据列表
	 ...
	 * @param apiId
	 */
	public DataList(Integer code,String message,String apiId){
		this.code=code;
	 ...
	 * @param apiId
	 */
	public DataList(T t,String apiId){
	 ...
	 * @param apiId
	 */
	public DataList(List<T> rows,Integer totalCount,String apiId){
		this.rows=rows;
